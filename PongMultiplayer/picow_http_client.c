...
#define HOST "192.168.37.138"
#define PORT 5000
#define INTERVALO_MS 50
#define TIMEOUT_CONEXAO 30000
#define PLAYER_ID 2 // Mudar para 2 no segundo RP2040
...
void atualizar_leds(bool conectado) {
    if (PLAYER_ID == 1) {
...
void calibrar_joystick(uint16_t *min_x, uint16_t *max_x, uint16_t *min_y, uint16_t *max_y) {
    sleep_ms(4000); // Espera 4 segundos para o usuário mover o joystick
    ...
    sleep_ms(1000); // Espera 1 segundo antes de continuar
}
